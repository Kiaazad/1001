...
init python:
    class fishing_class:
        ...
        def fish(self):
            if self.time:
                if self.time < .5:
                    fish = random.choice(self.variety)
                    hero.got(fish, 1)
                    self.population -= 1
                    self.time = 0
                    self.is_fishing = 0
                    ...
                    self.time = 0
        def cast(self):
            r = (1 + random.random()) - (float(self.population)/self.capacity)
...
screen fishing(p):
    ...
    vbox:
        spacing -180 align .5,.5
        button:
            background None
            add "fishing_bob" at fishing_caught(p.y)
